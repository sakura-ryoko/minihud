...
public abstract class ServuxHudHandler<T extends CustomPacketPayload> implements IPluginClientPlayHandler<T>
{
	...
	{
		@Override
		public void receive(ServuxHudPacket.@NonNull Payload payload, ClientPlayNetworking.@NonNull Context context)
		{
	...
	public <P extends IClientPayloadData> void decodeClientData(Identifier channel, P data)
	{
		...
		if (!HudDataManager.getInstance().isEnabled() || !this.checkFailures())
		{
			return;
		}
...
//							HudDataManager.getInstance().receiveRecipeManager((CompoundTag) fullPacket.readNbt(NbtAccounter.unlimitedHeap()));
				...
				default ->
						MiniHUD.LOGGER.warn("ServuxHudHandler#decodeClientData(): received unhandled packetType {} of size {} bytes.", packet.getPacketType(), packet.getTotalSize());
			}
		}
	}
	...
	public void tickFailures()
	{
		...
		{
			MiniHUD.debugLog("ServuxHudHandler#encodeClientData(): encountered [{}] sendPayload failures, cancelling any Servux join attempt(s)", this.maxFailures());
			this.servuxRegistered = false;
