...
public class RendererCallbacks {
    ...
    public static void onRandomTicksFixedToggled(IConfigBoolean config) {
        ...
        if (config.getBooleanValue() && entity != null) {
            ...
            String message = StringUtils.translate("minihud.message.toggled_using_position", config.getPrettyName(),
                    strStatus, strPos);
    ...
    public static void onSpawnChunksPlayerToggled(IConfigBoolean config) {
        if (config.getBooleanValue()) {
            ...
            message = StringUtils.translate("minihud.message.toggled_using_player_spawn", config.getPrettyName(),
                    strStatus, strDist);
    ...
    public static void onSpawnChunksRealToggled(IConfigBoolean config) {
        ...
        if (mc != null && mc.player != null) {
            if (config.getBooleanValue()) {
                ...
                // if (radius < 0)
                // {
                // HudDataManager.getInstance().setSpawnChunkRadius(2, true); // 1.20.5 Vanilla
                // Default
                // radius = 2;
                // }
                if (radius > 0) {
                    String strStatus = green + StringUtils.translate("malilib.message.value.on") + rst;
                    String strPos = String.format("x: %d, y: %d, z: %d [R: %d]", spawn.pos().getX(), spawn.pos().getY(),
                            spawn.pos().getZ(), radius);
                    message = StringUtils.translate("minihud.message.toggled_using_world_spawn", config.getPrettyName(),
                            strStatus, strPos);
