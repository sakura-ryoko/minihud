...
public class ShapeSquare extends ShapeCircleBase
{
	...
	public ShapeSquare()
	{
		super(ShapeType.SQUARE, Configs.Colors.SHAPE_SQUARE.getColor(), 16);
		this.hasData = false;
	...
	private void renderQuads(Vec3 cameraPos, Minecraft mc, ProfilerFiller profiler)
	{
		...
		profiler.push("square_quads");
		RenderObjectVbo ctx = this.renderObjects.getFirst();
		BufferBuilder builder = ctx.start(() -> "minihud:square/quads", this.renderThroughShape ? MaLiLibPipelines.MINIHUD_SHAPE_NO_DEPTH_OFFSET : MaLiLibPipelines.MINIHUD_SHAPE_OFFSET_NO_CULL);
		...
		this.renderSquareShapeQuads(cameraPos, builder);
		...
		{
			MiniHUD.LOGGER.error("ShapeSquare#renderQuads(): Exception; {}", err.getMessage());
		}
	...
	private void renderOutlines(Vec3 cameraPos, Minecraft mc, ProfilerFiller profiler)
	{
		...
		profiler.push("square_outlines");
		RenderObjectVbo ctx = this.renderObjects.get(1);
		BufferBuilder builder = ctx.start(() -> "minihud:square/outlines", MaLiLibPipelines.DEBUG_LINES_MASA_SIMPLE_LEQUAL_DEPTH);
		...
		this.renderSquareShapeOutlines(cameraPos, this.glLineWidth, builder);
		...
		{
			MiniHUD.LOGGER.error("ShapeSquare#renderOutlines(): Exception; {}", err.getMessage());
		}
	...
	private void renderSquareShapeQuads(Vec3 cameraPos, BufferBuilder builder)
	{
		LongOpenHashSet positions = new LongOpenHashSet();
		Consumer<BlockPos.MutableBlockPos> positionConsumer = this.getPositionCollector(positions);
		SphereUtils.RingPositionTest test = this::isPositionOnOrInsideSquare;
		BlockPos.MutableBlockPos mutablePos = new BlockPos.MutableBlockPos();
		Vec3d effectiveCenter = Vec3d.of(this.getEffectiveCenter());
	...
	private void renderSquareShapeOutlines(Vec3 cameraPos, float glLineWidth, BufferBuilder builder)
	{
		LongOpenHashSet positions = new LongOpenHashSet();
		Consumer<BlockPos.MutableBlockPos> positionConsumer = this.getPositionCollector(positions);
		SphereUtils.RingPositionTest test = this::isPositionOnOrInsideSquare;
	...
	protected boolean isPositionOnOrInsideSquare(int blockX, int blockY, int blockZ, Direction outSide)
	{
		Direction.Axis axis = this.mainAxis.getAxis();
		Vec3d effectiveCenter = Vec3d.of(this.getEffectiveCenter());
		...
		double dx = Math.abs(x - effectiveCenter.x);
		double dy = Math.abs(y - effectiveCenter.y);
		double dz = Math.abs(z - effectiveCenter.z);

		return dx <= radius + 1e-6 && dy <= radius + 1e-6 && dz <= radius + 1e-6;
	}
