...
public class ShapeSquare extends ShapeCircleBase
{
	...
	private void renderQuads(Vec3d cameraPos, Minecraft mc, ProfilerFiller profiler)
	{
		...
		BufferBuilder builder = ctx.start(() -> "minihud:square/quads", this.renderThroughShape ? MaLiLibPipelines.MINIHUD_SHAPE_NO_DEPTH_OFFSET : MaLiLibPipelines.MINIHUD_SHAPE_OFFSET_NO_CULL, 0);
	...
	private void renderOutlines(Vec3d cameraPos, Minecraft mc, ProfilerFiller profiler)
	{
		...
		BufferBuilder builder = ctx.start(() -> "minihud:square/outlines", MaLiLibPipelines.DEBUG_LINES_MASA_SIMPLE_LEQUAL_DEPTH, 0);
	...
	private void renderSquareShapeQuads(Vec3d cameraPos, BufferBuilder builder)
	{
	...
	private void renderSquareShapeOutlines(Vec3d cameraPos, float glLineWidth, BufferBuilder builder)
	{
		LongOpenHashSet positions = new LongOpenHashSet();
		Consumer<BlockPos.MutableBlockPos> positionConsumer = this.getPositionCollector(positions);
		SphereUtils.RingPositionTest test = this::isPositionOnOrInsideSquare;
