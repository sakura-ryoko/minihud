...
public class RenderObjectVbo
{
    ...
    protected RenderObjectVbo(Supplier<String> name, VertexFormat.DrawMode glMode, VertexFormat format, ShaderProgramKey shader, GlUsage usage)
    ...
    protected ShaderProgramKey getShader()
    {
        return this.shader;
    }

    protected Identifier getShaderId()
    {
        if (this.shader != null)
        {
            return this.shader.configId();
        }

        return Identifier.of(Reference.MOD_ID, "no_shader");
    }
    ...
    public BufferBuilder start(Supplier<String> name, VertexFormat.DrawMode glMode, VertexFormat format, ShaderProgramKey shader, GlUsage usage)
            throws RuntimeException
    {
        this.reset();
        this.name = name;
...
//    }
            ...
            throws RuntimeException
    {
        ...
        if (RenderSystem.isOnRenderThread())
        {
            ShaderProgram shader = RenderSystem.setShader(this.shader);
            this.vertexBuffer.bind();
            this.vertexBuffer.draw(modelView, projectionView, shader);
            VertexBuffer.unbind();
...
//            }
    ...
    {
        if (shader != null &&
            !this.shader.vertexFormat().equals(this.format))
        {
