...
public class InfoLineDolphinTreasure extends InfoLine
{
    ...
    {
        Triple<BlockPos, Integer, Boolean> dolphiPair = NbtEntityUtils.getDolphinDataFromNbt(nbt);
        List<Entry> list = new ArrayList<>();

        if (dolphiPair != null && entityType.equals(EntityType.DOLPHIN))
        {
            BlockPos treasure = dolphiPair.getLeft();
            int dryTime = dolphiPair.getMiddle();
            boolean hasTreasure = !treasure.equals(BlockPos.ORIGIN);

            // todo END 1.21.5+ (No treasure tags)
            if (dryTime == 2400)
            {
                // Submerged
                if (hasTreasure)
