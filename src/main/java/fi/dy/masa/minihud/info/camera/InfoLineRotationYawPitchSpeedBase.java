...
public abstract class InfoLineRotationYawPitchSpeedBase extends InfoLine
{
    ...
    public List<Entry> parse(@Nonnull InfoLineContext ctx)
    {
	    ...
	    {
			str.append(this.qt(ROT_YAW_KEY, MathHelper.wrapDegrees(ctx.ent().getYaw())));
		    pre = " / ";
	    }

	    if (InfoToggle.ROTATION_PITCH.getBooleanValue())
	    {
		    str.append(pre).append(this.qt(ROT_PITCH_KEY, MathHelper.wrapDegrees(ctx.ent().getPitch())));
		    pre = " / ";
	    }

	    if (InfoToggle.SPEED.getBooleanValue())
	    {
		    double dx = ctx.ent().getX() - ctx.ent().prevX;
		    double dy = ctx.ent().getY() - ctx.ent().prevY;
		    double dz = ctx.ent().getZ() - ctx.ent().prevZ;
